...
|---|---|---|---|
| `QSDStorageWriteErrorBurst` | warning      | 5m | [§3.1](#31-mode-a--QSDstoragewriteerrorburst) |
| `QSDStorageReadyFailing`    | **critical** | 2m | [§3.2](#32-mode-b--QSDstoragereadyfailing)    |
...
- **`QSD_storage_op_total{op, result}`** — per-(operation,
...
### 3.1 Mode A — `QSDStorageWriteErrorBurst`
...
2. **Cross-check the throughput sentinel**:
   - `QSDNoTransactionsStored` firing concurrently → full
     ...
     and [`MINING_LIVENESS.md` §3.1](MINING_LIVENESS.md#31-mode-a--QSDminingchainstuck).
...
3. **Cross-check the wallet surface**:
   - `QSDWalletStorageErrorBurst` firing concurrently → the
...
### 3.2 Mode B — `QSDStorageReadyFailing`
