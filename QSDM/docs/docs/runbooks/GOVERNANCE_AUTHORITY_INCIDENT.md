...
|---|---|---|---|
| `QSDGovAuthorityVoteRecorded`     | info         | 5m | [§3.1](#31-mode-a--QSDgovauthorityvoterecorded) |
| `QSDGovAuthorityThresholdCrossed` | warning      | 1m | [§3.2](#32-mode-b--QSDgovauthoritythresholdcrossed) |
| `QSDGovAuthorityCountTooLow`      | **critical** | 5m | [§3.3](#33-mode-c--QSDgovauthoritycounttoolow) |
...
- **Three lifecycle counters** (per `op`):
  - `QSD_gov_authority_voted_total{op}` — bumps on
    every accepted vote (Mode A's source).
  - `QSD_gov_authority_crossed_total{op}` — bumps
...
- **`QSD_gov_authority_count`** — the live
...
1. **Identify the mode.** The alert name maps 1:1
   ...
   - `QSDGovAuthorityVoteRecorded` → vote stage
     (Mode A).
   - `QSDGovAuthorityThresholdCrossed` → cross
     stage (Mode B).
   - `QSDGovAuthorityCountTooLow` → activated
...
### 3.1. Mode A — `QSDGovAuthorityVoteRecorded`
...
### 3.2. Mode B — `QSDGovAuthorityThresholdCrossed`
...
### 3.3. Mode C — `QSDGovAuthorityCountTooLow`
...
- **Source files:**
  - [`pkg/monitoring/gov_metrics.go`](../../../source/pkg/monitoring/gov_metrics.go)
    — `QSD_gov_authority_*` counters + gauge
...
- **Prometheus series:**
  ...
  - `QSD_gov_authority_activated_total{op}` —
    ...
    gauge moves with it).
  - `QSD_gov_authority_count` — Mode C's source
    gauge.
  - `QSD_gov_authority_rejected_total{reason}` —
...
| ------------------------------------ | ---- | ------------ | -------------- |
| `QSDGovAuthorityVoteRecorded`       | A    | info         | §3.1           |
| `QSDGovAuthorityThresholdCrossed`   | B    | warning      | §3.2           |
| `QSDGovAuthorityCountTooLow`        | C    | **critical** | §3.3           |
