...
func (self *WasiEnvironment) ReadStdout() []byte {
	...
	length := int(C.wasi_env_read_stdout(self.inner(), (*C.char)(buffer), C.size_t(bufferSize)))
	if length <= 0 {
		return nil
	}
...
func (self *WasiEnvironment) ReadStderr() []byte {
	...
	length := int(C.wasi_env_read_stderr(self.inner(), (*C.char)(buffer), C.size_t(bufferSize)))
	if length <= 0 {
		return nil
	}
