# Example: fetch dashboard /api/metrics/prometheus and list QSD_submesh_* lines.
# Usage:
...
$lines = ($text -split "`r?`n") | Where-Object { $_ -match '^QSD_submesh_' }
if (-not $lines) {
    Write-Error "No QSD_submesh_* lines (JWT or METRICS_SECRET may be required)."
    exit 1
}
